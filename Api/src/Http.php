...
class Http
{
    ...
    //通过get方式获取数据
    public static function doGet($url, $timeout=60, $header="")
    {
    ...
    //通过POST方式发送数据
    public static function doPost($url, $post_data=array(), $timeout=60, $header="")
    {
    ...
    public function file_get_contents($url, $params=array())
    {
    ...
    //通过curl post数据
    public static function curlPost($url, $post_data=array(), $timeout=60, $header="")
    {
        $header=empty($header)?"":$header;
    ...
    //通过socket get数据
    public static function socketGet($url, $timeout=60, $header="")
    {
        $header=empty($header)?"":$header;
        $url2 = parse_url($url);
    ...
    //通过socket post数据
    public static function socketPost($url, $post_data=array(), $timeout=60, $header="")
    {
        $header=empty($header)?"":$header;
        $post_string = http_build_query($post_data);
    ...
    public static function phpPost($url, $post_data=array(), $timeout=10, $header="")
    {
        $header=empty($header)?self::defaultHeader():$header;
        $post_string = http_build_query($post_data);
